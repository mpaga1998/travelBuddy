...
function validateTripInput(input) {
  ...
  if (!data.arrival || typeof data.arrival !== "object") {
    ...
    if (arrival.date === void 0 || arrival.date === null) {
  ...
  if (!data.departure || typeof data.departure !== "object") {
    ...
    if (departure.date === void 0 || departure.date === null) {
  ...
  if (data.arrival && typeof data.arrival === "object" && isValidISODate(data.arrival.date) && data.departure && typeof data.departure === "object" && isValidISODate(data.departure.date)) {
  ...
  if (data.desiredAttractions === void 0 || data.desiredAttractions === null) {
    errors.push({
  ...
  if (data.stops !== void 0 && data.stops !== null && !Array.isArray(data.stops)) {
    errors.push({
      field: "stops",
  ...
  if (data.travelPace !== void 0 && data.travelPace !== null) {
  ...
  if (data.interests !== void 0 && data.interests !== null && !Array.isArray(data.interests)) {
    errors.push({
      field: "interests",
  ...
  if (data.budget !== void 0 && data.budget !== null) {
  ...
  if (data.notes !== void 0 && data.notes !== null && typeof data.notes !== "string") {
    errors.push({
      field: "notes",
      message: "notes must be a string if provided"
    });
  }
  if (data.userFirstName !== void 0 && data.userFirstName !== null && typeof data.userFirstName !== "string") {
    errors.push({
      field: "userFirstName",
      message: "userFirstName must be a string if provided"
    });
  }
  if (data.userId !== void 0 && data.userId !== null && typeof data.userId !== "string") {
    errors.push({
      field: "userId",
...
function normalizeTripInput(input) {
  ...
  return {
    ...
    desiredAttractions: input.desiredAttractions.map((a2) => a2.trim()).filter(Boolean),
    travelPace: input.travelPace || "moderate",
    interests: cleanArray(input.interests),
    budget: input.budget || void 0,
...
function computeNights(arrival, departure) {
  const millisPerDay = 1e3 * 60 * 60 * 24;
...
function buildTripContext(input) {
...
function buildPlanningPrompt(context) {
  ...
  const attractionsList = desiredAttractions.length > 0 ? desiredAttractions.map((a2) => `- ${a2}`).join("\n") : "(No specific attractions requested)";
  const stopsSuggestion = stops && stops.length > 0 ? `Suggested stops: ${stops.join(", ")}.` : "No specific stops requested.";
  const returnLogistics = sameArrivalDepartureLocation ? `Return to ${departureLocation} by evening of ${endDate} for departure.` : `Return to ${departureLocation} by evening of ${endDate} for departure (requires travel from final location).`;
...
// server/lib/repairPrompt.ts
function buildRepairPrompt(context, invalidPlan, validationErrors) {
  const errorSummary = validationErrors.map((e2) => `- ${e2.path}: ${e2.message}`).join("\n");
...
function validatePlan(plan, expectedTotalNights) {
  const errors = [];
  ...
  const p2 = plan;
  if (typeof p2.isFeasible !== "boolean") {
    errors.push({ path: "isFeasible", message: "isFeasible must be a boolean (true or false)" });
  }
  if (!isNonEmptyString(p2.summary)) {
    errors.push({ path: "summary", message: "summary must be a non-empty string" });
  }
  if (!isPositiveInteger(p2.totalNights)) {
    errors.push({ path: "totalNights", message: "totalNights must be a positive integer" });
  } else if (p2.totalNights !== expectedTotalNights) {
    errors.push({
      path: "totalNights",
      message: `totalNights must equal ${expectedTotalNights}, got ${p2.totalNights}`
    });
  }
  if (!Array.isArray(p2.route)) {
    errors.push({ path: "route", message: "route must be an array" });
  } else {
    if (p2.route.length === 0) {
      errors.push({ path: "route", message: "route must have at least 1 stop" });
    }
    let routeNightSum = 0;
    const routeLocations = /* @__PURE__ */ new Set();
    p2.route.forEach((stop, index) => {
      const stopErrors = validateStop(stop, index, expectedTotalNights);
      errors.push(...stopErrors);
      if (stop && typeof stop === "object") {
        const s2 = stop;
        if (isNonEmptyString(s2.location)) {
          const loc = s2.location.toLowerCase();
          if (routeLocations.has(loc)) {
            errors.push({ path: `route[${index}].location`, message: `Duplicate location: ${s2.location}` });
          }
          routeLocations.add(loc);
        }
        if (isPositiveInteger(s2.nights)) {
          routeNightSum += s2.nights;
        }
      }
    });
    if (errors.filter((e2) => e2.path.startsWith("route")).length === 0 && isPositiveInteger(p2.totalNights)) {
      if (routeNightSum !== p2.totalNights) {
        errors.push({
          path: "route",
          message: `Sum of stop nights (${routeNightSum}) must equal totalNights (${p2.totalNights})`
        });
      }
    }
  }
  if (!Array.isArray(p2.transportSegments)) {
    errors.push({ path: "transportSegments", message: "transportSegments must be an array" });
  } else {
    p2.transportSegments.forEach((segment, index) => {
      const segErrors = validateTransportSegment(segment, index);
      errors.push(...segErrors);
    });
  }
  if (!isStringArray(p2.warnings)) {
    errors.push({ path: "warnings", message: "warnings must be an array of strings" });
  }
  if (!isStringArray(p2.cutsOrAlternatives)) {
...
function validatePlanLogic(plan, context) {
  const issues = [];
  ...
  if (!locationsMatch && finalStopLocation) {
    ...
    const hasReturnWarning = plan.warnings.some(
      (w2) => w2.toLowerCase().includes("return") || w2.toLowerCase().includes("depart")
    );
  ...
  const uniqueTransportFroms = new Set(plan.transportSegments.map((s2) => s2.from?.toLowerCase()));
  const routeLocations = new Set(plan.route.map((s2) => s2.location.toLowerCase()));
...
function formatBusinessIssues(issues) {
  ...
  let output = "";
  if (errors.length > 0) {
    output += "ERRORS:\n" + errors.map((i2) => `  [${i2.rule}] ${i2.message}`).join("\n");
  }
  if (warnings.length > 0) {
    if (output) output += "\n";
    output += "WARNINGS:\n" + warnings.map((i2) => `  [${i2.rule}] ${i2.message}`).join("\n");
  }
  return output;
}
async function attemptRepair(invalidPlan, validationErrors, context, openai2) {
  try {
    console.log(`\u{1F527} [Planner] Repair attempt 1/1: Sending validation context to model...`);
    const repairPrompt = buildRepairPrompt(context, invalidPlan, validationErrors);
    const repairResponse = await openai2.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [
        {
          role: "user",
      ...
      max_tokens: 2e3,
      temperature: 0.5
      // Lower temperature for deterministic corrections
    });
...
async function planItinerary(context, openai2) {
  try {
    console.log(
      `\u{1F3AF} [Planner] Starting plan generation for ${context.tripLengthCategory} trip (${context.totalNights} nights)`
    );
    ...
    if (!validationResult.valid) {
      ...
      if (repairedPlan) {
        ...
        const errorSummary = validationResult.errors.slice(0, 2).map((e2) => `${e2.path}: ${e2.message}`).join("; ");
    ...
    console.log("\u{1F3AF} [Planner] Validating business logic rules...");
...
${issueDetails}`);
    ...
    const businessErrors = businessValidation.issues.filter((i2) => i2.severity === "error");
    if (businessErrors.length > 0) {
      const errorSummary = businessErrors.map((e2) => `${e2.rule}: ${e2.message}`).join("; ");
    ...
    return {
      success: true,
      plan,
      businessIssues: businessValidation.issues.length > 0 ? businessValidation.issues : void 0
    };
  } catch (error) {
...
// server/lib/renderer.ts
function renderItinerary(input) {
  const { plan, context, travelerName } = input;
  let md = "";
  const greeting = travelerName ? `Hey ${travelerName}! Here's your itinerary for your ${context.tripLengthCategory} trip to Kyrgyzstan.` : `Here's your itinerary for your ${context.tripLengthCategory} trip to Kyrgyzstan.`;
  md += `# ${greeting}
...
`;
  md += `**Route**: ${plan.route.map((s2) => s2.location).join(" \u2192 ")}
`;
...
`;
  plan.route.forEach((stop, index) => {
    ...
    md += `### ${stop.location}
`;
    md += `Days ${stop.startDay}\u2013${stop.endDay} \xB7 ${stop.nights} night${stop.nights > 1 ? "s" : ""}
...
async function handler(req, res) {
  ...
  try {
    ...
    const response = {
      success: false,
      itinerary: "",
      error: isPlanningError ? `Could not plan itinerary: ${errorMessage}` : `Failed to generate itinerary: ${errorMessage}`
    };
