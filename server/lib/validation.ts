...
/**
 * Result of input validation. If valid=false, errors contains all issues found.
 */
 ...
 */
export function isValidISODate(dateString: unknown): boolean {
...
export function validateTripInput(input: unknown): ValidationResult {
  ...
  if (!data.departure || typeof data.departure !== 'object') {
    ...
    const departure = data.departure as Record<string, unknown>;
  ...
  if (
    ...
    isValidISODate((data.arrival as Record<string, unknown>).date) &&
    data.departure &&
    typeof data.departure === 'object' &&
    isValidISODate((data.departure as Record<string, unknown>).date)
  ) {
    const arrivalDate = (data.arrival as Record<string, unknown>).date as string;
    const departureDate = (data.departure as Record<string, unknown>).date as string;
