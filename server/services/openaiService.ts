...
export async function generateItinerary(input: TripInput): Promise<string> {
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      model: 'gpt-4-turbo', // Use GPT-4 for better JSON compliance
      messages: [
        {
          role: 'system',
          content:
    ...
    console.error('⚠️ Structured generation failed, falling back:', error instanceof Error ? error.message : error);
