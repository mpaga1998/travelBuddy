...
export function MapView() {
  ...
  useEffect(() => {
    ...
    container.innerHTML = `
      ...
      ${
        pin.imageUrls && pin.imageUrls.length > 0
          ? `<div style="margin-bottom:10px; position:relative; display:inline-block;">
               <img src="${escapeHtml(pin.imageUrls[0])}" style="width:200px;height:150px;object-fit:cover;border-radius:8px;cursor:pointer;display:block;" data-lightbox-url="${escapeHtml(pin.imageUrls[0])}" class="pin-image-preview" />
               ${
                 pin.imageUrls.length > 1
                   ? `<div style="position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.7);color:white;padding:4px 8px;border-radius:6px;font-weight:bold;font-size:14px;">+${pin.imageUrls.length - 1}</div>`
                   : ""
               }
               ${
                 pin.imageUrls.length > 1
                   ...
                   : `<div style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;opacity:0;background:rgba(0,0,0,0.5);border-radius:8px;transition:opacity 0.2s;pointer-events:none;" class="pin-image-hover">👁️ Click here to see more!</div>`
               }
             </div>`
          : ""
      }
      ...
      ${
        pin.tips && pin.tips.length > 0
          ? `<div data-tips-postit style="position:fixed;width:110px;height:110px;background:linear-gradient(135deg, #ffd966 0%, #fff2a8 50%, #ffd966 100%);border-radius:2px;box-shadow:0 3px 8px rgba(0,0,0,0.25), inset -1px -1px 0 rgba(0,0,0,0.1);padding:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;text-align:center;font-size:11px;font-weight:bold;color:#333;transform:rotate(-4deg);transition:opacity 0.2s;border:1px solid rgba(0,0,0,0.05);" data-tip-visible="true">
               📌 Click to view tips!
             </div>`
          : ""
      }
      ...
      <div style="display:flex; gap:8px;">
        ...
        <button data-dislike style="padding:8px 10px; border-radius:10px; border:1px solid rgba(0,0,0,0.18); background:white; cursor:pointer; font-weight:800; color:#111;">
          💔 <span style="margin-left:6px;">${pin.dislikesCount}</span>
        </button>
    ...
    // Add click handler for tips post-it and position it
    const tipsPostit = container.querySelector<HTMLDivElement>("[data-tips-postit]");
    if (tipsPostit) {
      // Position post-it so its center is at the top-right corner of popup
      const positionPostit = () => {
      ...
      // Reposition when popup moves
      popup.on('move', positionPostit);

      tipsPostit.addEventListener("click", (ev) => {
        ev.stopPropagation();
        setViewerTips(pin.tips ?? []);
        setCheckedTips(new Set());
        setTipsViewerOpen(true);
        // Hide the post-it when viewer opens
        tipsPostit.style.display = 'none';
        // Get popup position for tips viewer positioning
        const popupElement = popup.getElement();
        if (popupElement) {
          const rect = popupElement.getBoundingClientRect();
          sessionStorage.setItem('popupX', rect.left.toString());
          sessionStorage.setItem('popupY', rect.top.toString());
          sessionStorage.setItem('popupWidth', rect.width.toString());
        }
      });
    }
  ...
  return (
    <div style={{ display: "grid", gridTemplateRows: "auto 1fr", height: "100vh" }}>
      ...
      >
        ...
        >
          <div style={{ fontWeight: 700, whiteSpace: "nowrap" }}>Backpacker Map</div>
      ...
      <div style={{ position: "relative", height: "100%" }}>
            ...
            >
              ...
              <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
                ...
                <div>
                  ...
                  >
                    📷 Add your pictures here! ({draft.images.length}/5)
                    <input
                      type="file"
                      accept="image/*"
                      multiple
        ...
        {tipsViewerOpen && viewerTips.length > 0 && (
          <div
            style={(() => {
              const popupX = sessionStorage.getItem('popupX');
              const popupY = sessionStorage.getItem('popupY');
              const popupWidth = sessionStorage.getItem('popupWidth');
              const x = popupX ? parseInt(popupX) + (popupWidth ? parseInt(popupWidth) : 420) + 16 : window.innerWidth - 320;
              const y = popupY ? parseInt(popupY) : 100;
              return {
                position: "fixed" as const,
                left: `${x}px`,
                top: `${y}px`,
                width: "280px",
                background: "linear-gradient(135deg, #ffd966 0%, #fff2a8 50%, #ffd966 100%)",
                borderRadius: 4,
                padding: 16,
                boxShadow: "0 6px 16px rgba(0,0,0,0.25), inset -1px -1px 0 rgba(0,0,0,0.1)",
                color: "#333",
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {viewerTips.map((tip, idx) => (
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: 12, opacity: checkedTips.has(idx) ? 0.6 : 1, textDecoration: checkedTips.has(idx) ? "line-through" : "none" }}>
                    {tip}
                  </span>
                </label>
              ))}
