  ...
  return (
    ...
    <div className="absolute inset-x-0 bottom-24 flex justify-center px-4 pointer-events-none z-10">
      <div className="pointer-events-auto w-full max-w-[340px] bg-white rounded-2xl shadow-md border border-black/[0.08] p-4 flex flex-col gap-3">
        <div>
          <p className="text-sm font-semibold text-ink">It&apos;s quiet here.</p>
        ...
        <div className="grid grid-cols-1 min-[480px]:grid-cols-2 gap-2">
          <button
            type="button"
            onClick={onDropPin}
            className="px-3 py-2 rounded-xl border border-black/[0.18] bg-white text-sm font-medium text-ink hover:bg-gray-50 active:scale-[0.98] transition-transform"
          >
            📍 Drop the first pin
          </button>
          <button
            type="button"
            onClick={onPlanTrip}
            className="px-3 py-2 rounded-xl bg-brand text-white text-sm font-medium hover:bg-brand-600 active:scale-[0.98] transition-transform"
          >
            🗺️ Plan a trip here
          </button>
        </div>
